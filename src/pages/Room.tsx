      ...
      {token && (
        ...
        >
          <div className="flex-1 flex flex-col h-full relative overflow-hidden">
            <header className="h-16 shrink-0 flex items-center justify-between px-6 bg-black/80 backdrop-blur-xl border-b border-white/5 z-50">
              <div className="flex items-center gap-3 min-w-0">
                <button onClick={() => navigate('/dashboard')} className="shrink-0 p-2.5 hover:bg-white/5 rounded-xl transition-all">
                  <ArrowLeft className="w-4 h-4" />
                </button>
                <div className="min-w-0">
                  <h1 className="text-base font-black tracking-tight truncate">{roomData.name}</h1>
              ...
              <div className="flex items-center gap-2 shrink-0 ml-2">
                ...
                {(isHost || isAdmin) && joinRequests.length > 0 && (
                  <div className="relative">
                    <button
                      ...
                      className="p-2.5 rounded-xl border bg-yellow-500/10 border-yellow-500/20 text-yellow-400 hover:bg-yellow-500/20 transition-all relative"
                    >
                ...
                {(isHost || isAdmin) && (
                  <button onClick={() => setShowManager(!showManager)} className={`p-2.5 rounded-xl border transition-all flex items-center gap-1.5 text-[9px] font-black uppercase tracking-wider ${showManager ? 'bg-purple-600 border-purple-500 text-white' : 'bg-white/5 border-white/5 text-zinc-400 hover:bg-white/10'}`}>
                    <Settings2 className="w-4 h-4" />
                    <span className="hidden sm:inline">Video</span>
                  </button>
                )}
                {(isHost || isAdmin) && (
                   <button onClick={async () => { if(window.confirm('Xonani yopish?')) { await supabase.from('rooms').delete().eq('id', roomId); navigate('/dashboard'); } }} className="p-2.5 bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white rounded-xl border border-red-500/20 transition-all">
            ...
            <main className="flex-1 relative flex flex-col overflow-hidden bg-black/40 p-3 sm:p-4">
              <div className="flex-1 flex flex-col min-h-0 relative rounded-[32px] overflow-hidden shadow-2xl">
                ...
                {!roomData.video_url && !showManager && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center p-12 text-center z-0 bg-[#0a0a0a]">
                    <div className="w-20 h-20 bg-purple-500/5 rounded-full flex items-center justify-center mb-6 border border-purple-500/10"><Loader2 className="w-8 h-8 text-purple-500 animate-spin" /></div>
                    <h2 className="text-2xl font-black mb-3">Video Kutilmoqda...</h2>
                    <p className="text-zinc-500 text-sm">{isHost || isAdmin ? 'Video qo\'shish tugmasini bosing.' : 'Host video qo\'yishini kuting.'}</p>
                  </div>
                )}
              </div>
              ...
              {(isHost || isAdmin) && (
                <div className={`absolute bottom-0 left-0 right-0 z-[60] transition-all duration-500 ease-in-out ${showManager ? 'translate-y-0' : 'translate-y-full'}`}>
                  <div className="bg-[#0a0a0a] border-t border-white/10 p-6 shadow-[0_-30px_60px_rgba(0,0,0,0.8)] rounded-t-[32px]">
                    <div className="max-w-6xl mx-auto h-[420px] flex flex-col">
                      <div className="flex justify-between items-center mb-6 shrink-0">
                        <div>
                           <h3 className="text-xs font-black uppercase tracking-[0.2em] text-zinc-300">Video Boshqaruv Paneli</h3>
                           <p className="text-[10px] text-zinc-500 font-black uppercase tracking-widest mt-1">Video manbasini tanlang yoki yuklang</p>
                        </div>
              ...
              {showInvite && (
                <div className="absolute bottom-0 left-0 right-0 z-[60] transition-all duration-300">
                  <div className="bg-[#0a0a0a] border-t border-white/10 p-5 shadow-[0_-30px_60px_rgba(0,0,0,0.8)] rounded-t-[32px]">
                    <div className="max-w-3xl mx-auto">
                      <div className="flex justify-between items-center mb-5 shrink-0">
                        <div>
                          <h3 className="text-xs font-black uppercase tracking-[0.2em] text-zinc-300">Do'stlarni Taklif Qilish</h3>
                          <p className="text-[10px] text-zinc-500 font-black uppercase tracking-widest mt-1">Xona havolasini ulashing</p>
                        </div>
                      ...
                      <div className="flex gap-2 mb-5">
                        <input
                          ...
                          className="flex-1 bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-xs text-zinc-400 font-mono focus:outline-none"
                        />
                        <button
                          onClick={() => {
                            navigator.clipboard.writeText(`${window.location.origin}/room/${roomId}`);
                          }}
                          className="px-4 py-2.5 bg-purple-600 hover:bg-purple-500 text-white rounded-xl text-xs font-black transition-all flex items-center gap-1.5"
                        >
                          <Send className="w-3 h-3" /> Nusxa
                        </button>
                      </div>
                          ...
                          <div className="space-y-2">
                            {joinRequests.map(req => (
                              <div key={req.id} className="flex items-center justify-between p-3 bg-yellow-500/5 border border-yellow-500/10 rounded-xl">
                                <span className="text-sm font-bold">{req.profile?.display_name || req.profile?.email}</span>
